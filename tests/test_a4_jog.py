...
def test_jog_forward_dry_run_sends_manual_forward_then_stop() -> None:
    results = A4Pump(name="IN", port="COM_TEST", dry_run=True).jog_forward(1000)
...
def test_jog_reverse_dry_run_sends_manual_reverse_then_stop() -> None:
    results = A4Pump(name="IN", port="COM_TEST", dry_run=True).jog_reverse(1000)
...
def test_jog_duration_out_of_range_errors(duration_ms: int) -> None:
    with pytest.raises(ValueError, match="duration_ms"):
        A4Pump(name="IN", port="COM_TEST", dry_run=True).jog_forward(duration_ms)
