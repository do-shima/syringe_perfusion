...
def test_canonical_pep440_and_human_version_consistency() -> None:
    assert app_info.package_version() == "0.2.0b1"
    assert app_info.human_version() == "0.2.0-beta.1"
    assert app_info.future_tag_name() == "v0.2.0-beta.1"
    assert app_info.APP_VERSION == "0.2.0-beta.1"
    assert 'version = "0.2.0b1"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    for path in (
        ROOT / "README.md",
        ROOT / "CHANGELOG.md",
        ROOT / "docs" / "releases" / "v0.2.0-beta.1.md",
    ):
        assert "0.2.0-beta.1" in path.read_text(encoding="utf-8")
...
def test_source_fallback_and_embedded_build_info_loading(tmp_path: Path, monkeypatch) -> None:
    ...
    assert loaded["human_version"] == "0.2.0-beta.1"
...
def test_cli_version_has_traceable_fields(capsys) -> None:
    ...
    assert "A4PumpControl 0.2.0-beta.1" in output
    assert "commit " in output
    assert "control compatibility 1" in output
    assert "package 0.2.0b1" in output
...
def test_validation_schema_v1_read_and_build_identity_report(tmp_path: Path) -> None:
    ...
    assert record["package_version"] == "0.2.0b1"
    ...
    assert "Package version: 0.2.0b1" in report
