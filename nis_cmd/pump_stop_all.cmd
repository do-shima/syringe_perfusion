...
echo [%DATE% %TIME%] START pump_stop_all >> "%LOG%"
if not exist "%A4%" (echo ERROR: missing %A4% >> "%LOG%" & exit /b 2)
if not exist "%CFG%\pumps.json" (echo ERROR: missing pumps.json >> "%LOG%" & exit /b 3)
"%A4%" --config-dir "%CFG%" stop-all --dish-id NIS --condition stop_all --trigger-source NIS >> "%LOG%" 2>&1
set "RC=%ERRORLEVEL%"
echo [%DATE% %TIME%] END pump_stop_all EXIT=%RC% >> "%LOG%"
exit /b %RC%
